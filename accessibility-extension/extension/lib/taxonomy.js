...
// (customProfiles.siteTypes), the popup's site-type picker, and — from
// Phase 1 — the Librarian's category-scoped memories.
//
// Classic script (assigns globalThis.AA_TAXONOMY): loaded by background.js
// via importScripts and by extension pages via <script> tag. Keep it
// dependency-free.
//
...
// classifications are keyed by these ids, so Phase 1's migration/compaction
// logic uses the version to detect and repair out-of-vocabulary scopes.

globalThis.AA_TAXONOMY = {
  version: 2,
  ...
  // contain a video). Reserved in v2 for the Phase 1/2 scope chain
  // (`context:video` etc.); listed here so the vocabulary has one home.
  contexts: [
  ...
  // fall back to Gemini and should cache the answer (Phase 1 site index).
